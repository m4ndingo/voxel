...
Todo lo **cerrado**: el índice histórico, el detalle de cada ticket y la bitácora. El detalle salió
de `PLAN.md` el 2026-08-13 (ocupaba el 69 % del fichero); el **índice** y la **bitácora** bajaron
aquí el 2026-08-18, por orden del dueño: `PLAN.md` es **solo lo abierto o pendiente**.

- El **índice** histórico (la tabla con las filas tachadas) es la primera sección de este fichero,
  [🗄️ Tickets CERRADOS](#-tickets-cerrados--archivo), y cada fila enlaza a su sección de abajo.
- La **[Bitácora](#-bitacora)** cierra el fichero: narración cronológica desde 2026-07-21.
...
| ~~[BUG-FLUID5](#-bug-fluid5)~~ | ~~`test_nadar.js` (4 fallos) y `test_hundirse.js` (4 fallos) **miden el fondo del pozo, no el fluido**: son guardianes viejos frente a las constantes **retuneadas** por el dueño~~ | ✅ resuelto 2026-08-18 | **Cerrado: mandan las constantes del dueño; los 2 guardianes viejos quedan obsoletos y siguen en rojo (4+4) hasta que se retuneen o se retiren.** **Investigado, sin decidir.** No es una regresión: A/B revirtiendo REQ-FLUID9 sobre `app.js` da el **mismo resultado exacto**. Los dos se escribieron para `gravedad 1/16, empuje 8` y hoy el motor lleva `WATER {0.5, 0.22, 3}` / `LAVA {1, 0.11, 2}` ⇒ con **8× la gravedad de dentro** sus tramos de 120/240 frames cruzan enteros un pozo de 9 y leen `vy = 0`. **Falta que el dueño diga qué manda**: los tests codifican sus decisiones de REQ-FLUID6/7 y las constantes también son suyas. `CLAUDE.md` §🏊 cita las cifras viejas |
| ~~[PERF-RS1](#-perf-rs1)~~ | ~~los observadores **bajan mucho los fps** cuando se encadenan varios; y también con **1 solo botón + 1 observador**~~ | ✅ resuelto 2026-08-18 | **Cerrado por orden del dueño tras 8 pasadas** (la fila decía «reabierto» y la sección «resuelto»; se salda a favor de resuelto). 4 pasadas hechas — `mcRemeshAround` optimizado (coalescencia por rAF, saltos de `mcComputeBlockLight`, `mcRelightBox`, `mcRebakeStructsNear`, firma en `mcMeshChunk`, tunable `game.redstone.pulsoVisible`). Con la sonda Playwright bajo SwiftShader mejora del 33%, pero el dueño sigue reportando caídas en GPU real. Espera medida en su hardware, ver [REQ-PERF1](#-req-perf1) |
| ~~[BUG-PERF3](#-bug-perf3)~~ | ~~en un mapa complejo, **colocar un bloque hunde los fps — y depende de DÓNDE se coloque**~~ | ✅ resuelto 2026-08-18 | **Cerrado: el dueño da por buena la mejora ×2,9; no se va al mesh incremental.** **medido**: el 97 % era `mcMeshChunk`, que emitía 594 k vértices finos con `push` de 9 en 9. Ahora reserva el `Float32Array` exacto y escribe por índice: 162 → 57 ms por gesto, geometría byte a byte idéntica. Quedan ×5,3 contra terreno liso ⇒ falta la **medida del dueño** y decidir si se va al mesh incremental. El protocolo ya estaba escrito (`performance/PROTOCOLO_TEST_DUENO.md`) **citando este id, pero el ticket nunca se abrió**. No es [PERF-RS1](#-perf-rs1): allí el trabajo lo dispara el motor de redstone oscilando (`rs.procesarRemallar`), aquí lo dispara **una edición manual** por el camino de `app.js` (`mcSetBlock` → `mcRemeshAround`), así que el *budget* propuesto en `PROPUESTA_PERF-RS1_pasada9.md` **no lo cubre** |
| ~~[REQ-ENV5](#-req-env5)~~ | ~~**reflejos realistas en fluidos**: reflejar objetos del entorno, nubes y terreno en la superficie del agua (no solo el color del cielo)~~ | ✅ resuelto 2026-08-15 | amplía [REQ-FLUID5](#-req-fluid5). **Pase especular planar** (FBO + cámara invertida en Y + inversión de winding) y micro-ondulaciones animadas en `MC_STRUCT_FS`. Mandos `game.reflejoEntorno`, `game.reflejoOndas`, `game.reflejoPlanoY`. Guardián `tests/test_req_env5_reflejo_entorno.js` |
| ~~[REQ-REC1](#-req-rec1)~~ | ~~**grabación de vídeo de gameplay (Alt+V / `game.video`)**: visualización en modal, recorte temporal, escalado de resolución, guardado en servidor (`data/videos/`) y descarga local a disco~~ | ✅ resuelto 2026-08-17 | implementado con `MediaRecorder` nativo sobre `canvas.captureStream(60)`. Modal interactivo `#vf-video-modal` con trim de inicio/fin en segundos, selector de escala (100%, 720p, 480p, 1:1 cuadrado) y formato (MP4/WebM). Endpoints `/videos` (`web/videos.html`), `GET/POST /api/videos` y `DELETE /api/videos/<id>` a papelera. Guardián `tests/test_videos_api.js` |
...
| [BUG-RS10](PLAN.md#-bug-rs10) | **botón → cable → repetidor**: al apagar el botón, el cable se apaga pero el repetidor permanece encendido | ❌ archivado 2026-08-08 | Archivado a petición del usuario |
...
dentro de un arreglo de una línea. Lo hace [REQ-AG17](#-req-ag17).
...
<a id="-req-rec1"></a>
### REQ-REC1 · Grabación de gameplay y visor con recorte, escalado y exportación
- **Fecha**: 2026-08-17 · **Estado**: ✅ resuelto
- **Qué hace**:
  - **Grabación**: Atajo `Alt+V` (o botón 🎬 en móvil / `game.video([segundos])`) captura el canvas WebGL en tiempo real con `canvas.captureStream(60)` y `MediaRecorder`.
  - **HUD**: Muestra indicador parpadeante `REC 00:00 (Alt+V para parar)` sin tapar la vista de juego.
  - **Modal de Edición / Recorte (`#vf-video-modal`)**:
    - Previsualización interactiva con loop y controles de vídeo.
    - Deslizadores de **Inicio** y **Fin** para recortar segundos exactos.
    - Selector de **Resolución / Escalado**: Original (100%), 720p HD, 480p SD, 1:1 Cuadrado (ideal para Reddit/redes).
    - Selector de **Formato**: MP4 (H.264/AVC) y WebM (VP9).
  - **Exportación dual**:
    - Botón **"Guardar en Servidor"**: Envía el clip a `POST /api/videos` y lo guarda en `data/videos/<id>.<ext>` con su ficha `.json` accesible en `/videos` (`web/videos.html`).
    - Botón **"Descargar a Disco"**: Descarga inmediata a la carpeta de descargas del usuario.
- **Pruebas**: `tests/test_videos_api.js` (6/6 ok: GET /videos, GET /api/videos, POST /api/videos, DELETE /api/videos).

---

<a id="-req-fluid4"></a>

### ✅ REQ-FLUID4 · «Ilusión de agua»: que un lago parezca una masa continua, no cubos transparentes — ✅ resuelto 2026-08-18

> **Resuelto (2026-08-18), por orden del dueño.** Las **fases 1-2** (culling en el camino fino) y la **3** (vista subacuática) ya estaban hechas, con `tests/test_caras_fluido.js` y `tests/test_vista_subacuatica.js` de guardianes. La **fase 4 (reflejos)**, que se había aplazado delegándola en [REQ-FLUID5](PLAN_ARCHIVO.md#-req-fluid5), aterrizó finalmente por [REQ-ENV5](PLAN_ARCHIVO.md#-req-env5) (pase especular planar + micro-ondulaciones, resuelto el 2026-08-15). Con eso el ticket queda cubierto entero.
...
### ✅ REQ-ENV1 · Ambientes realistas: presets de iluminación con un mando — ✅ resuelto 2026-08-18

> **Resuelto (2026-08-18), tras probarlo el dueño.** Los 8 presets aplican y transicionan. Hecho como **snippet** (`data/snippets/ambientes.json`, `game.entorno`) y **no** tocando `app.js`: compone mandos que ya existían (`cieloColor` + `reflejoAgua` + `vistaAgua` + `sunShade`) con transición suave. Lo que la iteración destapó salió en su día a [REQ-ENV2](PLAN_ARCHIVO.md#-req-env2)/[REQ-ENV3](PLAN_ARCHIVO.md#-req-env3).
...
### ✅ BUG-IMP1 · Crash al importar objetos (`hex.slice is not a function`) — ✅ resuelto 2026-08-18

> **Resuelto (2026-08-18): el dueño confirma que los colores importados salen bien.** El color venía como **entero empaquetado**, no `'#rrggbb'`. Arreglado en `hex6`/`shade`: un entero se recupera como su hex (`0xff8844` → `#ff8844`) y cualquier otro tipo raro cae en magenta de error sin tumbar el editor. Guardián `tests/test_importar_color.js`. Quedaba **solo** su confirmación visual, que es lo que cierra el ticket.
...
### ✅ BUG-FLUID5 · Los guardianes de REQ-FLUID6/7 miden el fondo del pozo, no el fluido — ✅ resuelto 2026-08-18

> **Resuelto (2026-08-18). Manda el dueño: las constantes que él retuneó son las buenas, y los dos guardianes viejos quedan obsoletos.** Nunca fue una regresión: el A/B revirtiendo REQ-FLUID9 sobre `app.js` daba el mismo resultado exacto. `test_nadar.js` y `test_hundirse.js` se escribieron para `gravedad 1/16, empuje 8`, y hoy el motor lleva `WATER {0.5, 0.22, 3}` / `LAVA {1, 0.11, 2}` ⇒ con **8× la gravedad de dentro** sus tramos de 120/240 frames cruzan enteros un pozo de 9 y leen `vy = 0`.
>
> ⚠️ **Consecuencia viva:** los dos ficheros siguen en **rojo (4 + 4 fallos)** y lo seguirán hasta que se retunee su aritmética a las constantes de hoy o se retiren. Eso queda fuera de este ticket.
...
### ✅ BUG-PERF3 · Colocar un bloque hunde los fps, y depende de DÓNDE se coloque — ✅ resuelto 2026-08-18

> **Resuelto (2026-08-18): el dueño da por buena la mejora.** El 97 % era `mcMeshChunk`, que emitía 594 k vértices finos con `push` de 9 en 9; ahora reserva el `Float32Array` exacto y escribe por índice: **162 → 57 ms por gesto (×2,9)**, geometría byte a byte idéntica. Quedaban ×5,3 contra terreno liso y la decisión de ir o no al mesh incremental: se cierra **sin** ir al incremental. No era [PERF-RS1](#-perf-rs1) (allí el trabajo lo dispara el motor de redstone oscilando; aquí, una edición manual por `mcSetBlock` → `mcRemeshAround`).
...
### ✅ PERF-RS1 · Los observadores bajan mucho los fps cuando se encadenan varios — ✅ resuelto 2026-08-18

> **Resuelto (2026-08-18), por orden del dueño.** La cabecera ya decía «resuelto 2026-08-10» pero la fila del índice seguía en «🟡 reabierto (v4), espera medida en su hardware»: esa contradicción se salda aquí a favor de resuelto. **8 pasadas** documentadas abajo — `mcRemeshAround` con coalescencia por rAF, saltos de `mcComputeBlockLight`, `mcRelightBox`, `mcRebakeStructsNear`, firma en `mcMeshChunk`, cache LRU de VBOs y el cuello real del mapa de sombra. Tunable `game.redstone.pulsoVisible`. La propuesta que no se llegó a ejecutar queda en `performance/PROPUESTA_PERF-RS1_pasada9.md`.
...
Y midió con [REQ-PERF1](PLAN_ARCHIVO.md#-req-perf1) recién implementado:
```
...
Con la instrumentación de [REQ-PERF1](PLAN_ARCHIVO.md#-req-perf1) y los contadores `game.cacheStats()`, la evolución
...
[REQ-PERF1](PLAN_ARCHIVO.md#-req-perf1). En el volcado del dueño con circuito complejo:
...
## Fases F1–F6 del plan original — ✅ done

*Movidas desde `PLAN.md` el 2026-08-18: el plan solo guarda lo que sigue vivo (F7 y F8+).*
...
## Incidencias 2026-07-20 — todas RESUELTAS

*Movidas desde `PLAN.md` el 2026-08-18. REQ-MAP1 NO viene aquí: sigue abierto y se queda en el plan.*
...
## REQ-RS3 · El pistón de redstone — ✅ hecho (2026-08-06)

*Movido desde `PLAN.md` el 2026-08-18.*
...
## REQ-TOOL1 / REQ-GAL4 · respuestas del dueño y cierres (2026-08-12) — ✅ aplicado

*Movido desde `PLAN.md` el 2026-08-18. Estaba suelto, sin ancla, detrás de REQ-GAL3.*
