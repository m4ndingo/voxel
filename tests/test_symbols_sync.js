// @area: general
// @necesita: node
// Guardián de SYMBOLS.md, el mapa `función → app.js:línea` que existe para NO leer app.js entero
...
//
// El mapa se genera con:  grep -nE '^[[:space:]]*(async )?function ' app.js
...
const appLineas = fs.readFileSync(path.join(raiz, 'app.js'), 'utf8').split('\n');
const esperado = [];
...
// 2. Lo que dice SYMBOLS.md: filas `app.js:<línea>  <nombre>` (se ignora la cabecera con `#`)
...
const RE_FILA = /^app\.js:(\d+)[ \t]+([A-Za-z_][A-Za-z0-9_]*)/;
const enMapa = [];
symTexto.split('\n').forEach(l => {
...
for (const c of claves) {
  ...
  comprueba(real && fila && real.linea === fila.linea,
    c, real ? ('app.js:' + real.linea + (fila ? '' : ' — FALTA en el mapa')) : 'no está en app.js');
}

if (fallos) {
  console.log('\n⚠️  El mapa está desincronizado. Regenéralo desde /root/voxel con:');
  console.log("    grep -nE '^[[:space:]]*(async )?function ' app.js | \\");
  console.log("      sed -E 's/^([0-9]+):[[:space:]]*(async )?function ([A-Za-z_][A-Za-z0-9_]*).*/app.js:\\1  \\3/'");
